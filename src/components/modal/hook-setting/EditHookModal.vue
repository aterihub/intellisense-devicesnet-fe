     ...
     <div class="modal-inner" v-show="isOpen" ref="target">
       ...
       <VeeForm  v-slot="{ handleSubmit }" as="div" ref="form" >
         <form  @submit="handleSubmit($event, onSubmit)" class="form-wrapper" >
           <div class="rounded-lg bg-[#f2f2f2] px-3 py-2">{{ props.formData.module }}</div>
           <BaseInput v-model="props.formData.url" name="url" type="text" placeholder="Input webhook url" class="outlined" label="Name" required/>
...
import { onClickOutside } from '@vueuse/core'
  ...
  const { status, isLoading } = storeToRefs(useHookStore())
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    if (regButtonClick.value == 2) {
      ...
      await hookStore.updateHook(props.formData.module,values)
